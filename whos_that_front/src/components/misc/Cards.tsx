...
export const CardLayout = ({
    ...
    return (
        <figure
            className={`border ${isGame || isOppCard ? "max-xl:h-66.75 max-xl:w-39 h-80 w-52" : "max-xl:h-34 w-41.5 h-63.75 max-xl:w-24"} rounded-xs flex flex-col justify-between overflow-hidden ${isOppCard ? "animate-[flash-attention_2s_ease-in-out_1] border-orange-300 bg-orange-300" : "border-neutral-600 bg-white hover:scale-105"} text-center`}
        >
            <img
                className={`max-h-8/10 h-8/10 mx-1.5 mt-1.5 border border-neutral-400 bg-neutral-200 object-fill ${isOppCard ? "grayscale-75" : ""}`}
                src={imgSrc}
                alt={name}
            />
            <figcaption
                className={` ${isGame || isOppCard ? "items-center text-lg max-md:text-base" : "items-center text-base hover:text-blue-500"} max-h-1/10 m-auto flex text-center font-semibold leading-none text-black`}
            >
                {name}
            </figcaption>
    ...
    return (
        <>
            ...
            >
                ...
                <div className="mx-1 mb-1 box-content flex items-center justify-between border-neutral-900">
                    {/* Guess Button */}
                    <button
                        className={`w-[34%] cursor-pointer rounded bg-green-600 px-0.5 py-px text-center font-semibold text-neutral-100 transition-all hover:bg-green-700 max-2xl:text-sm max-sm:font-medium ${flipped ? "hidden" : ""}`}
                        onClick={() => {
                    ...
                    {/* Hide/Unhide Button */}
                    <button
                        className="w-[34%] cursor-pointer rounded bg-red-600 px-0.5 py-px text-center font-semibold text-neutral-100 transition-all hover:bg-red-700 max-2xl:text-sm max-sm:font-medium"
                        onClick={() => {
                            setFlipped(!flipped);
                        }}
                    >
