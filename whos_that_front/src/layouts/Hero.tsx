...
const Hero = () => {
    ...
    return (
        <header className="bg-radial flex w-full items-center justify-between rounded-b-[40%] from-blue-400 to-blue-600 px-[12%]">
            ...
            <h1 className="my-1 w-fit grow text-center">
                <Link
                    to={"/"}
                    id="title"
                    reloadDocument={true}
                    className="font-digitag text-shadow-md/100 cursor-pointer text-[7rem] font-semibold leading-none tracking-wide text-orange-300 hover:text-orange-400"
                >
                    W<span />
                    ho
            ...
            <div className="flex w-1/4 justify-end">
                <h2 className="inline-block">
                    <Link
                        ...
                        className="text-shadow-sm/25 text-xl font-bold text-white hover:text-blue-200"
                    >
                        <span className="text-shadow-sm text-2xl">
                            {isPending ? "" : session ? "🙋‍♂️ " : "👤 "}
                        </span>
                        <button className="active:translate-y-0.5">
                            {isPending ? "" : (session?.user.displayUsername ?? "Sign Up")}
                        </button>
                    </Link>

                    <button
                        className="px-2.25 w-fill border-b-5 border-x-1 text-shadow-xs/40 active:border-b-1 active:shadow-2xs active:inset-shadow-md duration-2 ml-10 cursor-pointer rounded-md border-cyan-500 bg-cyan-400 py-1 text-xl font-bold text-white shadow-md transition-all hover:border-cyan-600 hover:bg-cyan-500 active:translate-y-[1px]"
                        onClick={() => {
